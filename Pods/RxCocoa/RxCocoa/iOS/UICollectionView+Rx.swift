...
extension Reactive where Base: UICollectionView {
    ...
    public func modelSelected<T>(_ modelType: T.Type) -> ControlEvent<T> {
        let source: Observable<T> = itemSelected.flatMap { [weak viewShowDetail = self.base as UICollectionView] indexPath -> Observable<T> in
            guard let viewShowDetail = viewShowDetail else {
                return Observable.empty()
            }

            return Observable.just(try viewShowDetail.rx.model(at: indexPath))
        }
    ...
    public func modelDeselected<T>(_ modelType: T.Type) -> ControlEvent<T> {
        let source: Observable<T> = itemDeselected.flatMap { [weak viewShowDetail = self.base as UICollectionView] indexPath -> Observable<T> in
            guard let viewShowDetail = viewShowDetail else {
                return Observable.empty()
            }
